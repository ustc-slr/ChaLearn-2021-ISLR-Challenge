...
with open('results/AUTSL_I3D_depth2_all.pkl', 'rb') as f:
    avg_score_depth = pickle.load(f)
with open('results/AUTSL_I3D_depth_all.pkl', 'rb') as f:
    avg_score_depth2 = pickle.load(f)
...
for video_name, video_info in avg_score_i3d.items():
    ...
    pred = (pred_rgb + avg_score_i3d2[video_name][1] * 1.0) / 2.0 \
           + (avg_score_slowfast[video_name][1] + avg_score_slowfast2[video_name][1]) / 2.0 \
           + (avg_score_lhand[video_name][1] + avg_score_rhand[video_name][1]) * 1 \
           + avg_score_face[video_name][1] * 0.9 \
           + (avg_score_flow[video_name][1] + avg_score_flow2[video_name][1]) / 2.0 * 0.9 \
           + (avg_score_pose[video_name][1] + avg_score_pose2[video_name][1]) / 2.0 * 0.9 \
           + (avg_score_depth[video_name][1] + avg_score_depth2[video_name][1]) / 2.0 * 0.5
...
with open('predictions_fusion-final-RGBD.csv', 'w') as f2:
