...
if __name__ == '__main__':
    ...
    # for RGB
    print('RGB phase')
    a = torch.load('../pre_trained/model_rgb.pth')
    # tmp = np.load('v_CricketShot_g04_c01_rgb.npy')
    # input = torch.from_numpy(tmp)
    # real_input = input.permute(0,4,1,2,3)
    model = I3D(400)
    model.load_state_dict(a)
    # print(real_input.shape)
    real_input = torch.rand(5,3,64,224,224)
    output  = model(real_input)
    print(output.shape)
    print(torch.argmax(output), torch.max(output))
    # model.replace_logits(51)
    # for flow
    print('flow phase')
    a = torch.load('../pre_trained/model_flow.pth')
    model = I3D(400, modality='flow')
    model.load_state_dict(a)
    real_input = torch.rand(5, 2, 64, 224, 224)
    output = model(real_input)
    print(output.shape)
