...
class InceptionI3d(nn.Module):
    ...
    def __init__(
        ...
        self.avgpool = torch.nn.AvgPool3d((2, 7, 7), (1, 1, 1))
...
if __name__ == '__main__':
    ...
    model.replace_logits(1064)
    model = model.cuda()
    pretrain = torch.load('../pre_trained/i3d_bsl_model.pth.tar')
    model = nn.DataParallel(model)
    model.load_state_dict(pretrain['state_dict'])
    print(model)
    input = torch.rand(4, 3, 64, 224, 224)
    out = model(input)
    print(out.shape)
