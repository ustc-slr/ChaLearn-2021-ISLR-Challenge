...
def inference_top_down_pose_model_multi(model,
                                  img_or_path_video,
                                  person_results_video,
                                  bbox_thr=None,
                                  format='xywh',
    ...
    for person_results in person_results_video:
        ...
        with OutputHook(model, outputs=outputs, as_tensor=False) as h:
            for person_result in person_results:
                bbox_xyxy_list.append(person_result['bbox'])
                if format == 'xyxy':
                    try:
                        bbox_xyxy = np.expand_dims(np.array(person_result['bbox']), 0)
                    except:
                        print(person_results_video)
                ...
                if bbox_thr is not None:
                    ...
                    if bbox_xywh[0, 4] < bbox_thr:
                        continue
                bbox_xywh_list.append(bbox_xywh[0])

    valid_index_list = []
    unvalid_index_list = []
    valid_img_or_path_video = []
    valid_bbox_xywh_list = []
    for k in range(len(bbox_xywh_list)):
        if bbox_xywh_list[k] == []:
            unvalid_index_list.append(k)
            continue
        else:
            valid_index_list.append(k)
            valid_img_or_path_video.append(img_or_path_video[k])
            valid_bbox_xywh_list.append(bbox_xywh_list[k])

    if len(valid_bbox_xywh_list) != 0:
        pose_valid, heatmap = _inference_single_pose_model(
            model,
            valid_img_or_path_video,
            valid_bbox_xywh_list,
            dataset,
    ...
    pose = []
    index_valid = 0
    for k in range(len(img_or_path_video)):
        if k in valid_index_list:
            pose.append(pose_valid[index_valid])
            index_valid += 1
        else:
            pose.append(np.zeros((133, 3)))
    pose = np.stack(pose, axis=0)



    merge_result['keypoints'] = pose
